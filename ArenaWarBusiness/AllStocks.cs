...
namespace ArenaWarBusiness
{
  public class AllStocks : Script
  {
    ...
    private void OnTick(object sender, EventArgs e)
    {
      ...
      {
        this.SetDate();
        this.LoadIniFile2("scripts//HKH191sBusinessMods//ArenaWarBusiness//Main.ini");
        if (this.Config.GetValue<int>("Setup", "BuisnessLevel", this.purchaselvl) >= 1)
        {
          int num = new Random().Next(1, 1000);
          if (num < 500)
          {
            this.DecreaseGain("scripts//HKH191sBusinessMods//ArenaWarBusiness//Main.ini");
            if (this.ShowStockIncrease && (uint) this.Config.GetValue<int>("Setup", "BuisnessLevel", this.purchaselvl) > 0U)
            {
              if (!this.UseGlobalBusinessIncreaseDeceaseMessage)
                AllStocks.TextNotification("CHAR_LESTER", "~m~ Alan", "Arena War Business", "The Value of Stock has just Increased for our Arena War Business");
              if (this.UseGlobalBusinessIncreaseDeceaseMessage)
          ...
          {
            this.IncreaseGain("scripts//HKH191sBusinessMods//ArenaWarBusiness//Main.ini");
            if (this.ShowStockIncrease && (uint) this.Config.GetValue<int>("Setup", "BuisnessLevel", this.purchaselvl) > 0U)
            {
              if (!this.UseGlobalBusinessIncreaseDeceaseMessage)
                AllStocks.TextNotification("CHAR_LESTER", "~m~ Alan", "Arena War Business", "The Value of Stock has just Decreased for our Arena War Business");
              if (this.UseGlobalBusinessIncreaseDeceaseMessage)
                AllStocks.TextNotification("CHAR_LESTER", "~g~ Unknown", "All ~g~ HKH191's~w~ Business Mods", "The Value of Stock has just Increased/Decreased for our all Business");
            }
          }
