...
namespace MethBuisness
{
  public class AllStock : Script
  {
    ...
    public AllStock()
    {
      ...
      UI.Notify("~y~Biker Business~w~ Loaded, by ~g~HKH191~w~");
    ...
    public void IncreaseGain(string iniName)
    {
      ...
      this.stocksvalue += (float) (int) ((float) (random.Next((int) this.DecreaseStockMinAmount, (int) this.IncreaseStockMaxAmount) * this.purchaselvl) / 1.45f);
    ...
    private void OnTick(object sender, EventArgs e)
    {
      ...
      {
        this.SetDate();
        this.LoadIniFile2("scripts//HKH191sBusinessMods//MethBusiness//Main.ini");
        if (this.Config.GetValue<int>("Setup", "BuisnessLevel", this.purchaselvl) < 1)
          return;
        int num = new Random().Next(1, 1000);
        if (num < 500)
        {
          UI.Notify("Biker, Decrease");
          this.DecreaseGain("scripts//HKH191sBusinessMods//MethBusiness//Main.ini");
          if (this.ShowStockIncrease && (uint) this.Config.GetValue<int>("Setup", "BuisnessLevel", this.purchaselvl) > 0U)
          {
            if (!this.UseGlobalBusinessIncreaseDeceaseMessage)
              AllStock.TextNotification("WEB_CLAYPGJACKSON", "~y~ Clay", "Biker Business", "The Value of Stock has just Increased for our Biker Business");
            if (this.UseGlobalBusinessIncreaseDeceaseMessage)
              AllStock.TextNotification("CHAR_LESTER", "~g~ Unknown", "All ~g~ HKH191's~w~ Business Mods", "The Value of Stock has just Increased/Decreased for our all Business");
          }
        }
        else if (num >= 500)
        {
          UI.Notify("Biker, Increase");
          this.IncreaseGain("scripts//HKH191sBusinessMods//MethBusiness//Main.ini");
          if (this.ShowStockIncrease && (uint) this.Config.GetValue<int>("Setup", "BuisnessLevel", this.purchaselvl) > 0U)
          {
            if (!this.UseGlobalBusinessIncreaseDeceaseMessage)
              AllStock.TextNotification("WEB_CLAYPGJACKSON", "~y~ Clay", "Biker Business", "The Value of Stock has just Decreased for our Biker Business");
            if (this.UseGlobalBusinessIncreaseDeceaseMessage)
              AllStock.TextNotification("CHAR_LESTER", "~g~ Unknown", "All ~g~ HKH191's~w~ Business Mods", "The Value of Stock has just Increased/Decreased for our all Business");
          }
        }
    ...
    private void LoadIniFile3(string iniName)
    {
      try
      {
        ...
        try
        {
          ...
          this.UseGlobalBusinessIncreaseDeceaseMessage = this.Config3.GetValue<bool>("Setup", "UseGlobalBusinessIncreaseDeceaseMessage", this.UseGlobalBusinessIncreaseDeceaseMessage);
        ...
        this.BusinessUpgradeBasePrice = this.Config3.GetValue<float>("Prices", "BusinessUpgradeBasePrice", this.BusinessUpgradeBasePrice);
        this.IncreaseStockMinAmount = this.Config3.GetValue<float>("Prices", "IncreaseStockMinAmount", this.IncreaseStockMinAmount);
        this.IncreaseStockMaxAmount = this.Config3.GetValue<float>("Prices", "IncreaseStockMaxAmount", this.IncreaseStockMaxAmount);
        this.DecreaseStockMinAmount = this.Config3.GetValue<float>("Prices", "DecreaseStockMinAmount", this.DecreaseStockMinAmount);
        this.DecreaseStockMaxAmount = this.Config3.GetValue<float>("Prices", "DecreaseStockMaxAmount", this.DecreaseStockMaxAmount);
      }
      catch (Exception ex)
      {
        UI.Notify("~r~Error~w~: Config.ini Failed To Load.");
