...
namespace ExecutiveBusiness
{
  public class MissionScreen : Script
  {
    ...
    public void MissionPassed(string MissionName, int Payout, string Reason)
    {
      this.Fail = false;
      this.Pass = true;
      ...
      new UIResText(Reason, new Point(int32, 330), 0.5f, Color.White, GTA.Font.ChaletLondon, UIResText.Alignment.Centered).Draw();
    ...
    public void MissionPassed2(string MissionName, int Payout, int Payout2, float loss)
    {
      this.Fail = false;
      this.Pass = true;
      SizeF resolutionMantainRatio = UIMenu.GetScreenResolutionMantainRatio();
      int int32 = Convert.ToInt32(resolutionMantainRatio.Width / 2f);
      new Sprite("mpentry", "mp_modenotselected_gradient", new Point(0, 10), new Size(Convert.ToInt32(resolutionMantainRatio.Width), 550), 0.0f, Color.FromArgb(200, (int) byte.MaxValue, (int) byte.MaxValue, (int) byte.MaxValue)).Draw();
      new UIResText("mission passed", new Point(int32, 100), 2.5f, Color.FromArgb((int) byte.MaxValue, 199, 168, 87), GTA.Font.Pricedown, UIResText.Alignment.Centered).Draw();
      new UIResText(MissionName, new Point(int32, 230), 0.5f, Color.White, GTA.Font.ChaletComprimeCologne, UIResText.Alignment.Centered).Draw();
      new UIResText("Payout : $" + Payout.ToString("N"), new Point(int32, 300), 0.5f, Color.White, GTA.Font.ChaletLondon, UIResText.Alignment.Centered).Draw();
      new UIResText("Bonus : $" + Payout2.ToString("N"), new Point(int32, 330), 0.5f, Color.White, GTA.Font.ChaletLondon, UIResText.Alignment.Centered).Draw();
      new UIResText("Loss : " + loss.ToString() + "%", new Point(int32, 360), 0.5f, Color.White, GTA.Font.ChaletLondon, UIResText.Alignment.Centered).Draw();
      Scaleform scaleform = new Scaleform(0);
      scaleform.Load("instructional_buttons");
      scaleform.Render2D();
    }
    ...
    public void SetPass2(string MissionName, int Payout, string Reason)
    {
      ...
      for (int index = 0; index < 100; ++index)
      {
        ...
        if (this.Pass)
          this.MissionPassed(this.m, this.p, this.r);
        Script.Wait(1);
      }
    }
    ...
    public void SetPass2(string MissionName, int Payout, int Pay2, float Loss, string Reason)
    {
      Audio.SetAudioFlag("LoadMPData", true);
      Audio.PlaySoundFrontend("Mission_Pass_Notify", "DLC_HEISTS_GENERAL_FRONTEND_SOUNDS");
      Audio.SetAudioFlag("LoadMPData", false);
      ...
      for (int index = 0; index < 100; ++index)
      {
        if (this.Fail)
          this.MissionFailed(this.r, this.m);
        if (this.Pass)
          this.MissionPassed2(this.m, this.p, Pay2, Loss);
        Script.Wait(1);
      }
    }
